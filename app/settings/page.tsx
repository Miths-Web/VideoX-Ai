...
export default function SettingsPage() {
  const { currentUser: user, userProfile: profile, updateUserProfile, updateUserEmail, updateUserPassword, logout, updateUserData, loading: authLoading } = useAuth();
  ...
  useEffect(() => {
    const loadUserData = async () => {
      ...
      try {
        ...
        // Set from profile context
        const userData = profile;

        if (userData && userData.settings) {
          // Set notification preferences
          setEmailNotifications(userData.settings.emailNotifications ?? true);
          setProcessingAlerts(userData.settings.processingNotifications ?? true); // Mapped name
          setMarketingEmails(userData.settings.marketingEmails ?? false);

          // Set enhancement preferences
          setDefaultResolution(userData.settings.defaultResolution ?? "4k");
          setDefaultFps(userData.settings.defaultFps ?? "60");
          setAutoEnhanceColors(userData.settings.autoEnhanceColors ?? true);
          setAutoDenoising(userData.settings.autoDenoising ?? true);
          setAutoStabilization(userData.settings.autoStabilization ?? false);

          // Set storage settings
          setAutoDownload(userData.settings.autoDownload ?? false);
          setDeleteAfterDownload(userData.settings.deleteAfterDownload ?? false);
        }
      } catch (error) {
  ...
  const handleSaveProfile = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (user && email !== user.email) {
        toast.info("Email update requires verification. Check your inbox.");
      }
    ...
    try {
      ...
      await updateUserData({
        settings: {
          ...
          autoDenoising,
          autoStabilization
        } as any // Cast to any to avoid strict type checks if some fields are missing in partial
      });
    ...
    try {
      ...
      await updateUserData({
        settings: {
          ...currentSettings,
          emailNotifications,
          processingNotifications: processingAlerts,
          marketingEmails
        } as any
      });
  ...
  useEffect(() => {
    if (!authLoading && !user) {
      // Use window.location for a hard redirect to ensure no state leakage, or router for SPA feel
      // router.push("/auth/login");
      // Actually, let's stick to router for smoothness, but since we return null, it's safe.
      router.push("/auth/login");
    }
